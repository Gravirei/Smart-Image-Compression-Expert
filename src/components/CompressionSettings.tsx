...
export const CompressionSettings: React.FC<CompressionSettingsProps> = ({
  ...
  const handleCompress = async () => {
    try {
      ...
      setCompressionMessage('Compression failed, please try again');
    }
  };
  
  const getQualityLabel = (quality: number): string => {
    if (quality >= 0.9) return 'Maximum quality';
  ...
  return (
    <div className="space-y-6">
      <div className="text-center mb-6">
        ...
        <p className="text-gray-600">Adjust parameters for optimal compression results</p>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <label className="font-semibold text-gray-800">Target file size (optional)</label>
        </div>
        <div className="relative">
          <input
            type="text"
            placeholder="e.g: 1.5MB, 500KB"
        ...
        <p className="text-sm text-gray-500">
          Original size: {formatFileSize(originalImage.size)}
        </p>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <label className="font-semibold text-gray-800">Image Quality</label>
        </div>
        <div className="space-y-2">
          ...
          <div className="flex justify-between text-sm">
            <span className="text-gray-500">Smallest file</span>
            <span className="font-semibold text-blue-600">
              {(config.quality * 100).toFixed(0)}% - {getQualityLabel(config.quality)}
            </span>
            <span className="text-gray-500">Best quality</span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <label className="font-semibold text-gray-800">Output Format</label>
        </div>
        ...
        <div className="text-sm text-gray-500 space-y-1">
          <div>• JPEG: Smallest files, good for photos</div>
          <div>• PNG: Supports transparency, good for icons</div>
          <div>• WebP: New format, higher compression rate</div>
        </div>
      </div>
